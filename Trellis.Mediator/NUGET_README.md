...
- **Tracked-aggregate dispatch (opt-in)**: alternative `TrackedAggregateDomainEventDispatchBehavior<,>` (registered via `AddTrackedAggregateDomainEventDispatch()`) reads the aggregates the unit-of-work tracked at commit time and dispatches their events automatically — fires for any response shape, including outcome DTOs. Mutually exclusive with response-shape dispatch.
