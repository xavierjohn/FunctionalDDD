namespace RailwayOrientedProgramming.Tests.Functional.Results.Extensions
{
    ...
    public class BindTests_ValueTask_Right : BindTestsBase
    {
        ...
        {
            Result<K, Err> output = await Failure_T().BindAsync(Func_T_ValueTask_Success_K);
        ...
        {
            Result<K, Err> output = await Success_T(T.Value).BindAsync(Func_T_ValueTask_Success_K);
