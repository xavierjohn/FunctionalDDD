namespace RailwayOrientedProgramming.Tests.Functional.Results.Extensions
{
    ...
    public class BindTests_ValueTask_Right : OkTestsBase
    {
        [Fact]
        public async ValueTask Bind_ValueTask_Right_T_K_returns_failure_and_does_not_execute_func()
        {
            Result<K, Error> output = await Failure_T().IfOkAsync(Func_T_ValueTask_Success_K);
        ...
        {
            Result<K, Error> output = await Success_T(T.Value).IfOkAsync(Func_T_ValueTask_Success_K);
