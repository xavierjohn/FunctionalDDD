...
public class BindTests_ValueTask : BindTestsBase
{
    ...
    {
        Result<K, Err> output = await ValueTask_Failure_T().BindAsync(Func_T_ValueTask_Success_K);
    ...
    {
        Result<K, Err> output = await ValueTask_Success_T(T.Value).BindAsync(Func_T_ValueTask_Success_K);
