...
public class BindTests_Task : BindTestsBase
{
    ...
    {
        Result<K, Err> output = await Task_Failure_T().BindAsync(Func_T_Task_Success_K);
    ...
    {
        Result<K, Err> output = await Task_Success_T(T.Value).BindAsync(Func_T_Task_Success_K);
