...
public class EnsureTests_Task_Left
{
    ...
    {
        var result = Task.FromResult(Result.Failure<string, Error>(Error.Conflict("initial error message")));
    ...
    {
        var tResult = Task.FromResult(Result.Failure<string, Error>(Error.Conflict("initial error message")));

        var result = await tResult.EnsureAsync(x => x != "",
            x => Error.Validation("new error message: string should not be empty"));

        result.IsOk.Should().BeFalse("Input Result.Failure should be returned");
    ...
    {
        var tResult = Task.FromResult(Result.Success<string, Error>("initial ok"));

        var result = await tResult.EnsureAsync(x => x != "",
            x => Error.Validation("new error message: string should not be empty"));

        result.IsOk.Should().BeTrue("Input Result passes predicate condition");
    ...
    {
        var tResult = Task.FromResult(Result.Success<int?, Error>(null));

        Result<int?, Error> result = await tResult.EnsureAsync(value => !value.HasValue, value => Error.Validation($"should be null but found {value}"));
    ...
    {
        var tResult = Task.FromResult(Result.Success<string, Error>(""));

        var result = await tResult.EnsureAsync(x => x != "",
            x => Error.Validation("new error message: string should not be empty"));

        result.IsOk.Should().BeFalse("Input Result fails predicate condition");
    ...
    public async Task Ensure_Task_Left_with_successInput_and_successPredicate()
    {
        ...
        var result = await initialResult.EnsureAsync(() => Result.Success<string, Error>("Success message"));
    ...
    public async Task Ensure_Task_Left_with_successInput_and_failurePredicate()
    {
        ...
        var result = await initialResult.EnsureAsync(() => Result.Failure<string, Error>(Error.Unexpected("Error message")));
    ...
    {
        var initialResult = Task.FromResult(Result.Failure<string, Error>(Error.Unexpected("Initial Error message")));
    ...
    public async Task Ensure_Task_Left_with_successInput_and_parameterisedFailurePredicate()
    {
        ...
        var result = await initialResult.EnsureAsync(_ => Result.Failure<string, Error>(Error.Unexpected("Error Message")));
    ...
    public async Task Ensure_Task_Left_with_successInput_and_parameterisedSuccessPredicate()
    {
        ...
        var result = await initialResult.EnsureAsync(_ => Result.Success<string, Error>("Success Message"));
    ...
    public async Task Ensure_Task_Left_with_failureInput_and_parameterisedFailurePredicate()
    {
        ...
        var result = await initialResult.EnsureAsync(_ => Result.Failure<string, Error>(Error.NotFound("Success Message")));
