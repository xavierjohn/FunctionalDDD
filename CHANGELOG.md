...
- **`Trellis.Core`** — four new opt-out marker attributes (`[AllowEmpty]`, `[AllowWhitespace]`, `[NoTrim]`, `[AllowDefault]`) that are recognised by the generator in this release for future-compatibility but have no emission effect today. They become consumer-visible when the strict-default emission flip lands in the following release; declare them now to keep existing code lenient through that flip without code churn.
