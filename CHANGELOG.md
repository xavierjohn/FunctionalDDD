...
`CompositeValueObjectJsonConverter<T>.WritePrimitive` previously checked `raw is null` before validating the primitive type, so an unsupported interior shape with a null payload — `int? = null`, `string[] = null`, a null nested composite VO — silently serialized as JSON `null` while reading the same JSON threw `TrellisJsonValidationException: "Unsupported primitive type ..."`. The converter could therefore publish JSON it cannot itself parse back. RFC 9111 / ProblemDetails consumers downstream of such an endpoint would never see the load-bearing validation that the converter is supposed to provide on both directions.
