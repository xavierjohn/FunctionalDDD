...
The shared short↔long table is also extended in this release to cover the OAuth2 / OIDC / Microsoft identity-platform subset of `JwtSecurityTokenHandler.DefaultInboundClaimTypeMap` that consumers realistically configure as an `ActorIdClaim` or `PermissionsClaim`: `scp` ↔ `http://schemas.microsoft.com/identity/claims/scope` (OAuth scopes), `oid` ↔ `http://schemas.microsoft.com/identity/claims/objectidentifier` (Entra v2.0 actor id), `tid` ↔ `http://schemas.microsoft.com/identity/claims/tenantid`, `upn` ↔ `ClaimTypes.Upn`, `idp` ↔ `http://schemas.microsoft.com/identity/claims/identityprovider`, `acr` ↔ `http://schemas.microsoft.com/claims/authnclassreference`, `amr` ↔ `http://schemas.microsoft.com/claims/authnmethodsreferences`, plus the AD FS legacy aliases `adfs1email` and `adfs1upn`. Without these, `PermissionsClaim = "scp"` would have remained the same silent-403 trap under `MapInboundClaims = true`. Device, certificate, request-transport, and password-policy claims from the inbound map are intentionally not included: they are not authentication state and configuring them as an actor id or permission would be a misconfiguration in its own right.
