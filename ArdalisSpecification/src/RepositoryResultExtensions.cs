...
/// <summary>
/// Result-returning extensions for Ardalis.Specification's IRepositoryBase.
/// Provides seamless integration with Railway Oriented Programming patterns.
/// </summary>
public static class RepositoryResultExtensions
{
    ...
    /// <example>
    /// <code>
    /// var result = await repository.FirstOrNotFoundAsync(new OrderByIdSpec(orderId));
    /// // Returns Result&lt;Order&gt; - either Success(order) or Failure(NotFoundError)
    /// </code>
    /// </example>
    ...
    /// <example>
    /// <code>
    /// var result = await repository.SingleOrNotFoundAsync(new ActiveOrderByCustomerSpec(customerId));
    /// // Returns Result&lt;Order&gt;:
    /// // - Success(order) if exactly one found
