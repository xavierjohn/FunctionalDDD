...
- Code fix: appends `.WithVersionedRoute()` to the flagged `CreatedAtRoute(...)` (or `WithLocation(...)`) call (the `Trellis.Asp.ApiVersioning` extension) and adds `using Trellis.Asp.ApiVersioning;` when missing. The new `using` is inserted in the same scope as existing usings (file-scoped namespace, block-scoped namespace, or top-level).
