...
| `public TrellisServiceBuilder UseWorkerActor(Actor systemActor)` | `TrellisServiceBuilder` | Composes the existing unkeyed `IActorProvider` registration with a worker-actor wrapper via `AddTrellisWorkerActor(systemActor)`. The wrapper returns `systemActor` when `IHttpContextAccessor.HttpContext` is `null` (background-worker ticks, hosted services) and delegates to the inner provider otherwise. Applied AFTER any `UseCachingActorProvider<T>()` wrap so worker-tick lookups bypass the caching layer. Throws `InvalidOperationException` on repeated call, when the prior inner descriptor is singleton-lifetime via implementation type or factory (use `services.AddSingleton<IActorProvider>(instance)` or re-register as scoped instead), or when no `UseXxxActorProvider(...)` has been chained first. Keyed registrations are ignored. |
