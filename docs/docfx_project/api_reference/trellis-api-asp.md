...
| `WithLocation(string routeName, Func<TDomain, RouteValueDictionary> routeValues)` | `HttpResponseOptionsBuilder<TDomain>` | Adds a `Location` header to the response **without** changing the status code (typically 200 OK; or whatever `WriteOutcome.Replaced` / `Accepted` drives). Generated via `LinkGenerator.GetUriByName`. RFC 9110 §10.2.2 permits `Location` on any 2xx response that identifies a related resource — use this on state-transition endpoints that mutate an existing resource (e.g., `POST /orders/{id}/return` returning 200 OK). For new-resource creation use `CreatedAtRoute` instead. Same versioning trap as `CreatedAtRoute` — chain `.WithVersionedRoute()` from `Trellis.Asp.ApiVersioning` under query/header versioning. |
