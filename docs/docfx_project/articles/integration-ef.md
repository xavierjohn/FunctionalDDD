...
- **The `regenerate` callback returns a `ValueTask<bool>`.** Return `true` to continue; return `false` to abort the retry loop. Aborting leaves the conflicting entries detached (sibling aggregates remain tracked) and returns `Error.Conflict` with `"Retry aborted by regenerate callback"` detail.
- **`attempt` is 1-based and equals the regenerate-call number.** With `maxAttempts: 3` the call sequence on consecutive failures is: save → regenerate(attempt: 1) → save → regenerate(attempt: 2) → save → return `Error.Conflict` with `"Maximum retry attempts exhausted"` detail. The final attempt does **not** call regenerate.
...
- **Unrecognised `DbUpdateException` shapes are rethrown.** When `shouldRetry` returns false, the method maps FK → `"referential.integrity"`, duplicate → `"duplicate.key"`, and **rethrows** anything else (connection failures wrapped in `DbUpdateException`, trigger failures, provider-specific violations). This matches `SaveChangesResultAsync` semantics.
