...
| `ClaimsActorOptions.PermissionsClaim` | `"permissions"` | Multi-valued JWT claims arrive as repeated `Claim` instances and are aggregated via `FindAll`. Same well-known short↔long fallback as `ActorIdClaim`: literal match first, then every counterpart from the JWT inbound claim-name map is also queried and merged into a deduplicated set, so `"role"`/`"roles"`/`ClaimTypes.Role` all just-work against `JwtBearerOptions.MapInboundClaims = true` or `false`. The default `"permissions"` is not in the JWT inbound map, so it resolves by literal match only. Emits a debug-level log entry when the fallback fires. |
